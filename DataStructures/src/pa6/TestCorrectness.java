...
public class TestCorrectness {
    ...
    private static void testTrie() {
        ...
        for (int i = 0; i < 7; i++) trie.insert(dictionary[i]);
        ...
        System.out.print("\nIncorrect spellings: ");
        for (int i = 0; i < numWordsInDoc; i++) {
            if (!spellCheck[i]) System.out.print(document[i] + " ");
        }
    }
    ...
    private static void testLCS() {
        ...
        String str2 = "cbccabd";
        System.out.printf(
            "Longest Common Substring of %s and %s is %s%n",
            str1,
            str2,
    ...
    private static ArrayList<ArrayList<Integer>> arraysToLists(
        int[][] arrays,
        int arrayLengths[],
        int n
    ) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < arrayLengths[i]; j++) list.add(arrays[i][j]);
            lists.add(list);
        }
        return lists;
    }
    ...
    private static void testTopKElements() throws Exception {
        ...
        ArrayList<Student> all = PriorityQueueApplications.topK(
            students,
            students.size()
        );
    ...
    private static void testKSortedMerge() {
        ...
        for (int i = 0; i < k; i++) eachListLength[i] = lists[i].length;
        ArrayList<Integer> mergedList = PriorityQueueApplications.kWayMerge(
            arraysToLists(lists, eachListLength, k)
        );
        System.out.println("Original sorted arrays");
        for (int i = 0; i < k; i++) System.out.println(
            Arrays.toString(lists[i])
        );
        ...
        for (int i = 0; i < k; i++) eachListLength[i] = lists[i].length;
        mergedList =
            PriorityQueueApplications.kWayMerge(
                arraysToLists(lists, eachListLength, k)
            );
        System.out.println("Original sorted arrays");
        for (int i = 0; i < k; i++) System.out.println(
    ...
    private static void testDijkstra() throws Exception {
        ...
        for (int j = 0; j < filePaths.length; j++) {
            ...
            for (int i = 0; i < dijk.numVertices; i++) {
                dijk.execute(i);
                System.out.println(
                    "\nDistance array (from v" +
                    i +
                    "): " +
                    Arrays
                        .toString(dijk.distance)
    ...
    private static void testKruskal() throws FileNotFoundException {
        ...
        int mstWeight = 0;
        for (Edge e : mst) mstWeight += e.weight;
        System.out.printf(
            "MST has weight %d%nThe edges are: %s",
            mstWeight,
            mst
        );
    }
