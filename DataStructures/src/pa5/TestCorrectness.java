...
public class TestCorrectness {
    ...
    private static void testComponent() throws Exception {
        ...
        for (int i = 0; i < filePaths.length; i++) {
            System.out.printf(
                "*** Test Component Counter on Undirected Graph %d ***\n\n",
                i + 1
            );
            BFS cc = new BFS(filePaths[i]);
            System.out.println(
                "Number of components is " + cc.countComponents()
            );
            System.out.println();
        }
    }
    ...
    private static void testTransitiveClosure() throws Exception {
        String filePaths[] = {
            DIRECTED_GRAPH1_PATH,
            DIRECTED_GRAPH2_PATH,
            UNDIRECTED_GRAPH1_PATH,
            UNDIRECTED_GRAPH2_PATH,
        };
        int numFiles = 2;
        for (int i = 0; i < numFiles; i++) {
            System.out.printf(
                "*** Test Transitive Closure on Directed Graph %d ***\n\n",
                i + 1
            );
            BFS app = new BFS(filePaths[i]);
            boolean[][] tc = app.computeTransitiveClosure();
            for (int j = 0; j < app.numVertices; j++) System.out.println(
                Arrays.toString(tc[j])
            );
            System.out.println();
        }
        for (int i = 0; i < numFiles; i++) {
            System.out.printf(
                "*** Test Transitive Closure on Undirected Graph %d ***\n\n",
                i + 1
            );
    ...
    private static void testMutantLanguage() throws Exception {
        ...
        for (int i = 0; i < filePaths.length; i++) {
            ...
            if (topoOrder != null) System.out.println(
                "Alphabet order: " + Arrays.toString(topoOrder)
            ); else System.out.println(
                "Unfortunately, this language has circular dependency."
            );
            System.out.println();
        }
    }
