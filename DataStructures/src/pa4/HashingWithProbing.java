...
public class HashingWithProbing {
    ...
    public int search(int key) { // complete this method
        ...
        for (int i = 0; i < TABLE_SIZE; i++) {
            if (hashTable[hashValue] == key) return hashValue;
            if (hashTable[hashValue] == EMPTY) return -1; // we have reached an empty slot
            hashValue++; // increment the hash value
            if (hashValue == TABLE_SIZE) hashValue = 0; // we have reached the end of the array in the probe sequence // reset the hash value to 0
        }
    ...
    public int insert(int val) { // complete this method
        ...
        for (int i = 0; i < TABLE_SIZE; i++) {
            if (hashTable[hashValue] == val) return -1; // the key is already in the hash table
            if (hashTable[hashValue] == EMPTY) break; // we have reached an empty slot

            // Getting here means the current slot is full
            hashValue++; // Probe the next slot
            if (hashValue == TABLE_SIZE) hashValue = 0; // We have reached the end of the array in the probe sequence // wrap around to the beginning of the array
        }
    ...
    public int remove(int val) { // complete this method
        ...
        if (index < 0) return -1; // the key is not in the hash table
