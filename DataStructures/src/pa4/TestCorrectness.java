...
public class TestCorrectness {
    ...
    static final int numInsert = insertionArray.length;

    static final int searchArray[] = { 29, 3, 19, 27, 12, 34, 4, 5, 19, 20, 32, 45, 37, 25, 99, 25, 8, 24, 12, 16 };
    static final int numSearch = searchArray.length;
    //prettier-ignore	
	final static int deleteArray[] = { 11, 16, 12, 15, 5, 17, 19, 4, 5, 20, 32, 17, 19, 39, 99, 10, 8, 19, 15, 21, 0, 55 };
    static final int numDelete = deleteArray.length;

    static final int[] cleanUp = { 77, 65, 3, 66, 2, 88, 78, 39, 67 };
    static final int numCleanUp = cleanUp.length;

    static final int CHAINING_TABLE_SIZE = 7;
    static final int INITIAL_PROBING_TABLE_SIZE = 4;
    ...
    private static void printList(final List<Integer> list) {
        ...
        System.out.print("[");
        for (int i = 0; i < list.size() - 1; i++) System.out.print(it.next() + ", ");
    ...
    public static HashingWithChaining testChaining() throws Exception {
        ...
        for (int i = 0; i < numInsert; i++) hChain.insert(insertionArray[i]);
        ...
        for (int i = 0; i < numSearch; i++) {
            ...
            if (hChain.search(val)) foundList.add(val); else notFoundList.add(val);
        }
        ...
        for (int i = 0; i < numDelete; i++) {
            ...
            if (hChain.remove(val)) deleteList.add(val); else notFoundList.add(val);
        }
    ...
    public static HashingWithProbing testProbing() throws Exception {
        ...
        for (int i = 0; i < numSearch; i++) {
            int val = searchArray[i];
            if (hProbing.search(val) >= 0) foundList.add(val); else notFoundList.add(val);
        }
        System.out.print("Found: ");
        printList(foundList);
        System.out.print("\nDid not find: ");
        ...
        for (int i = 0; i < numDelete; i++) {
            int val = deleteArray[i];
            if (hProbing.remove(val) >= 0) deleteList.add(val); else notFoundList.add(val);
        }
        System.out.print("Deleted: ");
        printList(deleteList);
        System.out.print("\nDid not find: ");
    ...
    private static BST testBST() {
        ...
        for (int i = 0; i < numSearch; i++) {
            int val = searchArray[i];
            if (bst.search(val) != null) foundList.add(val); else notFoundList.add(val);
        }
        System.out.print("Found: ");
        printList(foundList);
        System.out.print("\nDid not find: ");
        ...
        for (int i = 0; i < numDelete; i++) {
            int val = deleteArray[i];
            if (bst.remove(val)) deleteList.add(val); else notFoundList.add(val);
        }
        System.out.print("Deleted: ");
        printList(deleteList);
        System.out.print("\nDid not find: ");
    ...
    private static void testBSTApplications() throws Exception {
        ...
        for (int key : KEY) {
            ...
            if (predecessor == Integer.MIN_VALUE) System.out.print("predecessor = -infty; "); else System.out.print(
                "predecessor = " + predecessor + "; "
            );
            if (successor == Integer.MAX_VALUE) System.out.print("successor = infty; "); else System.out.print(
                "successor = " + successor + "; "
            );
    ...
    private static void cleanTest(HashingWithChaining chaining, HashingWithProbing probing, BST bst) {
        ...
        int size = 0;
        for (int i = 0; i < CHAINING_TABLE_SIZE; i++) size += chaining.hashTable[i].size();
