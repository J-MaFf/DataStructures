...
public class BST {
    ...
    public BSTNode search(int key) { // Define the method, taking an integer key argument and returning a BSTNode
        ...
        while (tmp != null) { // Start a loop that runs while tmp is not null (i.e., there are still nodes to be searched)
            if (tmp.value == key) return tmp; // Check if the value of tmp is equal to the key // If so, return the node tmp
            else if (tmp.value > key) tmp = tmp.left; // If the value of tmp is greater than the key // Traverse the left subtree by setting tmp to tmp's left child
            else tmp = tmp.right; // Traverse the right subtree by setting tmp to tmp's right child
        }
    ...
    public boolean remove(int val) { // complete this method
        ...
        if (nodeToBeDeleted == null) return false; // If the node is not found, return false
