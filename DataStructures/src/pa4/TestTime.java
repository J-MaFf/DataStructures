...
public class TestTime {
    ...
    public static void compareHashingAndBST() throws Exception {
        ...
        for (int i = 0; i < U; i++) {
            ...
            if (!inserted) numFailedInsertions++;
            totalChainIns += System.currentTimeMillis() - startTime;

            startTime = System.currentTimeMillis();
            if (bst.insert(val) != null && !inserted) throw new Exception("Something wrong with insertion!");
            totalBSTIns += System.currentTimeMillis() - startTime;

            startTime = System.currentTimeMillis();
            if (hProbing.insert(val) >= 0 && !inserted) throw new Exception("Something wrong with insertion!");
        ...
        for (int i = 0; i < U; i++) {
            ...
            if (!found) numFailedSearches++;
            totalChainSearch += System.currentTimeMillis() - startTime;

            startTime = System.currentTimeMillis();
            if (null != bst.search(val) && !found) throw new Exception("Something wrong with search!");
            totalBSTSearch += System.currentTimeMillis() - startTime;

            startTime = System.currentTimeMillis();
            if (hProbing.search(val) >= 0 && !found) throw new Exception("Something wrong with search!");
        ...
        for (int i = 0; i < U; i++) {
            ...
            if (!deleted) numFailedDeletions++;
            totalChainDel += System.currentTimeMillis() - startTime;

            startTime = System.currentTimeMillis();
            if (bst.remove(val) && !deleted) throw new Exception("Something wrong with deletion!");
            totalBSTDel += System.currentTimeMillis() - startTime;

            startTime = System.currentTimeMillis();
            if (hProbing.remove(val) >= 0 && !deleted) throw new Exception("Something wrong with deletion!");
    ...
    private static void testBSTApplicationsTime() throws Exception {
        ...
        int array[] = new int[U];
        for (int i = 0; i < U; i++) array[i] = rand.nextInt(U);
        ...
        for (int i = 0; i < numSearches; i++) {
            ...
            if (Math.abs(nrViaAVL - key) != Math.abs(nrVal - key)) throw new Exception(
                "Oops! Wrong code for nearest neighbour"
            );
        ...
        System.out.println(
            "Total time over " +
            numSearches +
            " nearest neighbour searches using linear scan on " +
            U +
            " elements: " +
            totalTimeLinearScan_NNR +
            " (may vary with each execution)"
        );
        System.out.println(
            "Total time over " +
            numSearches +
            " nearest neighbour searches using BST on " +
            U +
            " elements: " +
            totalTimeAVL_NNR +
            " (may vary with each execution)"
